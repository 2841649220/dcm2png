...
from numba import jit, prange, vectorize, float32, int32, uint8
...
def process_single_dcm_file(dcm_path, output_dir):
    """处理单个DCM文件"""
    try:
        # 读取DICOM文件
...
def process_dcm_folder(input_folder, output_folder, max_workers=None):
    """使用多进程处理文件夹中的所有DCM文件"""
    # 确保输出目录存在
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_file = {
            executor.submit(process_single_dcm_file, dcm_file, output_folder): dcm_file
...
if __name__ == "__main__":
    ...
    process_dcm_folder(args.input_folder, args.output_folder, args.workers)
